/**
 * Seka Real Canvas Watermarking Engine & Export Utility
 * Handles client-side canvas compositing of text, stickers, and the official Seka watermark stamp.
 */
...
/**
 * Composites the canvas with meme text, stickers, and Seka's official watermark.
...
export async function createWatermarkedCanvas(options: CompositeOptions): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      // 4. Stamp Official Seka Watermark Badge (bottom-right)
    ...
    img.onerror = (err) => {
      ...
      if (ctx) {
        ...
        ctx.textAlign = 'center';
        ctx.fillText('SEKA MEME', 400, 400);
        stampSekaWatermark(ctx, 800, 800);
        resolve(canvas.toDataURL('image/png'));
      } else {
        reject(err);
      }
...
/**
 * Stamps the exact official Seka badge onto canvas ctx at bottom right.
 */
export function stampSekaWatermark(ctx: CanvasRenderingContext2D, width: number, height: number) {
  ...
  ctx.fillText('Seka', textX + boltWidth + 4, textY);
 ...
 */
export function triggerFileDownload(dataUrl: string, filename = 'seka-meme.png') {
