      ...
      <div className="flex border-b border-[#27272A] bg-[#0A0A0A]">
        <button
          ...
          className={`flex-1 py-3 text-xs font-black uppercase tracking-wider text-center border-b-2 transition-colors ${
            subTab === 'for_you' ? 'border-[#E6FF00] text-[#E6FF00]' : 'border-transparent text-[#71717A] hover:text-[#A1A1AA]'
          }`}
        >
          For You
        </button>
        <button
          ...
          className={`flex-1 py-3 text-xs font-black uppercase tracking-wider text-center border-b-2 transition-colors ${
            subTab === 'following' ? 'border-[#E6FF00] text-[#E6FF00]' : 'border-transparent text-[#71717A] hover:text-[#A1A1AA]'
          }`}
        >
          Following
        </button>
        <button
          ...
          className={`flex-1 py-3 text-xs font-black uppercase tracking-wider text-center border-b-2 transition-colors ${
            subTab === 'trending' ? 'border-[#E6FF00] text-[#E6FF00]' : 'border-transparent text-[#71717A] hover:text-[#A1A1AA]'
          }`}
        >
          Trending
        </button>
      ...
      <main className="p-4 max-w-lg mx-auto">
        {displayedMemes.length === 0 ? (
          <div className="text-center py-16 space-y-3">
            ...
            <button
              onClick={() => { setSelectedCategory('All'); setSubTab('for_you'); setSearchQuery(''); }}
          ...
          <>
            ...
            {subTab === 'for_you' && onLoadMore && hasMore && (
              <button
                onClick={onLoadMore}
                disabled={loadingMore}
                className="w-full py-3 rounded-full bg-[#18181B] border border-[#27272A] text-xs font-bold text-[#A1A1AA] hover:text-white hover:border-[#3F3F46] disabled:opacity-50 transition-colors"
              >
