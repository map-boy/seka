      ...
      <div className="bg-[#18181B] border-t border-[#27272A] rounded-t-3xl max-h-[80vh] flex flex-col shadow-2xl max-w-lg mx-auto w-full">
        <div className="p-4 border-b border-[#27272A] flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <h3 className="text-sm font-black text-white">Comments ({memeComments.length})</h3>
            <span className="text-[10px] bg-[#E6FF00]/15 text-[#E6FF00] px-2 py-0.5 rounded-full font-bold">Live</span>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-[#27272A] flex items-center justify-center text-[#A1A1AA] hover:text-white">
            <X className="w-4 h-4" />
          </button>
        </div>
          ...
          {memeComments.length === 0 ? (
            ...
            memeComments.map((comment) => (
              <div key={comment.id} className="flex space-x-3 items-start">
                <img src={comment.authorAvatar} alt={comment.authorName} className="w-8 h-8 rounded-full object-cover border border-[#27272A]" />
                ...
                <button
                  ...
                  className={`flex flex-col items-center pt-2 transition-transform active:scale-125 ${comment.isLiked ? 'text-[#E6FF00]' : 'text-[#71717A] hover:text-white'}`}
                >
        ...
        <div className="px-4 py-2 bg-[#0A0A0A] border-t border-[#27272A] flex items-center justify-around">
          ...
          {QUICK_EMOJIS.map((emoji) => (
            <button key={emoji} onClick={() => handleQuickEmoji(emoji)} className="text-lg hover:scale-125 transition-transform">
              {emoji}
            </button>
          ))}
        </div>
        ...
        <div className="p-3 bg-[#18181B] border-t border-[#27272A] flex items-center space-x-2">
          <img
            src={currentUserAvatar || 'https://api.dicebear.com/7.x/identicon/svg?seed=me'}
            alt="My Avatar"
